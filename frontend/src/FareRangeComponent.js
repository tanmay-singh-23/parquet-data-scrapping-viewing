// frontend/src/FareRangeComponent.js

import React, { useState } from 'react';
import axios from 'axios';
import './styles.css';

const FareRangeComponent = () => {
    const [trips, setTrips] = useState([]);
    const [minFare, setMinFare] = useState('');
    const [maxFare, setMaxFare] = useState('');

    const fetchTripsByFareRange = async () => {
        try {
            const response = await axios.get(`http://localhost:8000/trips/farerange?min=${minFare}&max=${maxFare}`);
    ...
    return (
        <div className="table-container">
            <h2>Trips by Fare Range</h2>
            <div className="inputstyle">
                <label>
                    Min Fare:
                    <input type="number" value={minFare} onChange={(e) => setMinFare(e.target.value)} />
                </label>
                <label>
                    Max Fare:
                    <input type="number" value={maxFare} onChange={(e) => setMaxFare(e.target.value)} />
                </label>
                <button onClick={fetchTripsByFareRange}>Fetch Trips</button>
            </div>
            <table>
                <thead>
                    <tr>
...
export default FareRangeComponent;
