// frontend/src/SharedTripsComponent.js

import React, { useState } from 'react';
import axios from 'axios';
import './styles.css';

const SharedTripsComponent = () => {
    const [trips, setTrips] = useState([]);

    const fetchSharedTrips = async () => {
        try {
            const response = await axios.get('http://localhost:8000/trips/sharedtrips');
    ...
    return (
        <div className="table-container">
            <h2>Shared Trips</h2>
            <button className = "sharedTripButton" onClick={fetchSharedTrips}>Fetch Trips</button>
            <table>
                <thead>
                    <tr>
...
export default SharedTripsComponent;
