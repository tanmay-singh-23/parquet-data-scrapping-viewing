// frontend/src/TipsRangeComponent.js

import React, { useState } from 'react';
import axios from 'axios';
import './styles.css';

const TipsRangeComponent = () => {
    const [trips, setTrips] = useState([]);
    const [minTips, setMinTips] = useState('');
    const [maxTips, setMaxTips] = useState('');
    ...
    return (
        <div className="table-container">
            <h2>Trips by Tips Range</h2>
            <div className="inputstyle">
                <label>
                    Min Tips:
                    <input type="number" value={minTips} onChange={(e) => setMinTips(e.target.value)} />
                </label>
                <label>
                    Max Tips:
                    <input type="number" value={maxTips} onChange={(e) => setMaxTips(e.target.value)} />
                </label>
                <button onClick={fetchTripsByTipsRange}>Fetch Trips</button>
            </div>
            <table>
                <thead>
                    <tr>
...
export default TipsRangeComponent;
